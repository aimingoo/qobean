...
Interface = function() {
  ...
  // return "foo is Qomo's constructor"
  function IsConstructor(foo) {
    return IsClass(foo.prototype.ClassInfo);
  }
  ...
  function isImplemented(intf) {
    ...
    if (this instanceof Function) {
      ...
      if (IsConstructor(this)) {
        while (i--) if (all[i] instanceof Function && ((this == all[i]) || (this.prototype instanceof all[i]))) return true;
      }
...
*/
  ...
  _Interface.WarpInterfaces = function() {
    for (var i=0, imax=arguments.length; i<imax; i++) warpInterface(arguments[i]);
  }
...
/**
 for classtypes: Delegate(_cls(), tbl)
 for classes   : Delegate(_cls().Create, tbl)
